...
public final class Ped4 extends MovementAlgorithm {
	...
	/**
	 * Klasa s³u¿y do zwracania informacji z funkcji calculateGap().
	 * 
	 * @author Pawel
	 * 
	 */
	...
	// bezwzglêdnie maksymalna iloœæ pól mo¿liwych do przejœcia w jednej
	// iteracji
	public static final int V_MAX = 6;

	// TODO: jak rozwi¹zaæ problem ró¿nych v_max dla ró¿nych ludzi?
		...
		for (int i = 1; i <= 2 * w.getvMax(); i++) {
			...
			if (op != null) {
				if (getDir(w.getDirection(), op.getDirection()) == Dir.OPP) {
					...
					// ORTHO te¿ traktowane jak SAME, bo bez ryzyka kolizji
					gap_same = i - 1;
				}
				break;
			}
			...
			for (int w = 0; w < board.getDimension().width; w++) {
				...
				whole: while (walk.getvCurr() > 0) {
					MyPoint dest = curr.add(walk.getDirection().getCoords()
							.mul(walk.getvCurr()));
					walk.setvCurr(walk.getvCurr() - 1); // SprawdŸ obie strony
														// pola...
